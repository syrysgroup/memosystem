...
import { getChannelOrgUnits, getCurrentProfile, getStaffDirectory } from "@/lib/data";

export default async function MessagesIndexPage() {
  const profile = await getCurrentProfile();
  if (!profile) return null;

  const [channels, staff] = await Promise.all([getChannelOrgUnits(profile.org_unit_id), getStaffDirectory()]);
