  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-4">
        <h2 className="mb-3 text-sm font-semibold text-slate-900">Look up a document for a visitor</h2>
        <form action={trackDocument} className="flex items-end gap-3">
          <div className="flex-1">
            <label className="text-sm font-medium text-slate-700">Unique code</label>
            <input
              name="unique_code"
      ...
      {positions.length === 0 ? (
        <p className="text-sm text-amber-700">You need an active Position to log incoming correspondence.</p>
      ) : (
        <form action={logIncomingLetter} className="space-y-3 rounded-lg border border-slate-200 bg-white p-6">
          ...
          {positions.length > 1 ? (
            <div>
              <label className="text-sm font-medium text-slate-700">Logging as</label>
              <select name="originating_position_id" required className="mt-1 w-full rounded-md border border-slate-300 px-3 py-2 text-sm">
                {positions.map((p) => (
